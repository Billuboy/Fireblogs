...
export default function LoginModal() {
  ...
  async function handleSubmit(e) {
    ...
    const res = await getDoc(doc(firestore, 'users', newUser.uid));
    setUser({
  ...
  return (
    <>
      <div
        className="w-126 h-48 bg-orange-200 rounded-full opacity-90 hover:opacity-100 cursor-pointer ml-8"
        onClick={openModal}>
            ...
            {newUser ? (
              <>
                <Dialog.Title as="h3" className="text-24 font-semibold">
                  Looks like you are new to FireBlogs
                </Dialog.Title>
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <button
                    type="submit"
                    className="bg-orange-200 w-126 h-32 text-18 font-medium grid place-items-center rounded-full disabled:opacity-70 block m-auto"
                    disabled={disabled}>
                    Register
                  </button>
                </form>
